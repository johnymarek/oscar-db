...
<backgroundDisplay>
	...
	<text offsetXPC=34 offsetYPC=8 widthPC=35 heightPC=10 fontSize=20 backgroundColor=-1:-1:-1 foregroundColor=51:153:255>
...
if ($db = new PDO("sqlite:$dbname")) {
	...
	if ($query === false) {
		...
		while ($row = $r->fetch(SQLITE_ASSOC)) {
			...
			echo "<link>http://127.0.0.1:82/oscar-db/list.php?g=".$tmpg."</link>\n";
			echo "</item>\n\n";
		  }
		...
		while ($row = $r->fetch(SQLITE_ASSOC)) {
			...
			echo "<link>http://127.0.0.1:82/oscar-db/list.php?c=".$tmpc."</link>\n";
			echo "</item>\n\n";
		  }
